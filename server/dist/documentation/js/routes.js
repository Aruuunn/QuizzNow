document.addEventListener('DOMContentLoaded', function () {
    ...
    function foundLazyModuleWithPath(path) {
        var split = path.split('#'), lazyModulePath = split[0], lazyModuleName = split[1];
    ...
    var nodes = d3.layout.hierarchy()(tree), svg = d3.select("#body-routes").append('svg'), svg_g = svg.append("g"), svg_p = svg.append("g"), last_id = 0, node = svg_g.selectAll(".node")
    ...
    var xmin = root_extents[0], ymin = root_extents[1], xmax = root_extents[2], ymax = root_extents[3], area_sum = (xmax - xmin) * (ymax - ymin), x_size_min = nodes[0].x_size, y_size_min = nodes[0].y_size;
    ...
    var links = svg_p.selectAll(".link")
        ...
        .attr("class", "link")
        .attr("d", diagonal);
    var _svg = document.getElementById('main'), main_g = _svg.childNodes[0];
